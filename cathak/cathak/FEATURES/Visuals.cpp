...
void CVisuals::DrawInaccuracy()
{
	...
	if (local_player->IsAlive())
	{
		...
		if (!weapon->is_grenade() && !weapon->is_knife())
			RENDER::DrawFilledCircle(cW, cH, accuracy + 3, 30, CColor(255, 0, 0,15));
	}
}
...
void CVisuals::DrawIndicator()
{
	...
	if (SETTINGS::settings.overrideenable)
	{
		...
		if (SETTINGS::settings.overridemethod == 0)
			RENDER::DrawF(10, screen_height - iY, FONTS::visuals_lby_font, false, false, SETTINGS::settings.overridething ? CColor(0, 255, 0) : CColor(255, 0, 0), "OV");
		else if (SETTINGS::settings.overridemethod == 1)
		{
			GetAsyncKeyState(UTILS::INPUT::input_handler.keyBindings(SETTINGS::settings.overridekey)) ? 
				RENDER::DrawF(10, screen_height - iY, FONTS::visuals_lby_font, false, false, CColor(0, 255, 0), "OV") : 
				RENDER::DrawF(10, screen_height - iY, FONTS::visuals_lby_font, false, false, CColor(255, 0, 0), "OV");
		}
	}
...
//	"         ", "        J", "       Je",
//	"      Jen", "     Jend", "    Jenda",
//	"   JendaW", "  JendaWa", " JendaWar",
//	"JendaWare", "endaWare", "ndaWare  ",
//	"daWare  ", "aWare    ", "Ware    ",
//	"are      ", "re       ", "e        "
//};
...
//			setClanTag(clantagAnim[autism], "JendaWare");
...
void CVisuals::GameSense()
{
	...
	if (GetTickCount() > lastTime)
	{
		...
		if (kek > 10) {
			autism++; if (autism > 18) autism = 0;
			setClanTag(GameSenseAnim[autism], "catto");
