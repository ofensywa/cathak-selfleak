...
private:
	...
	void UTIL_TraceLineSig(const Vector& vecAbsStart, const Vector& vecAbsEnd, unsigned int mask, const SDK::CBaseEntity *ignore, int collisionGroup, SDK::trace_t *ptr)
	{
		...
		static UTIL_TraceLine_t TraceLine = (UTIL_TraceLine_t)UTILS::FindSignature("client_congratsufoundantipaste.dll", "55 8B EC 83 E4 F0 83 EC 7C 56 52");
	...
	{
		static DWORD dwAddress = UTILS::FindPattern("client_zadey.dll", (BYTE*)"\x53\x8B\xDC\x83\xEC\x08\x83\xE4\xF0\x83\xC4\x04\x55\x8B\x6B\x04\x89\x6C\x24\x04\x8B\xEC\x81\xEC\x00\x00\x00\x00\x8B\x43\x10", "xxxxxxxxxxxxxxxxxxxxxxxx????xxx");
		if (!dwAddress)
			return;
